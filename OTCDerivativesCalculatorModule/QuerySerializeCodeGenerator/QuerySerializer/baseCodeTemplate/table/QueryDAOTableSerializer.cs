...
{
    public class QueryDAOTableSerializer
    {
        string path = @"D:\Project File\OTCDerivativesCalculatorModule\QuerySerializeCodeGenerator\QuerySerializer\baseCodeTemplate\codeTemplate\";

        private string className_ = "TEST_Class_Name";
        private string tableName_ = "TEST_Table_Name";
        ...
        private string staticQueryProperty()
        {
            ...
            for (int i = 0; i < qm_.ColumnList_.Count - 1; i++)
            {
                ...
                insertQuery_SB.Append(upperItem_insertColumn + ", ");
            }

            upperItem_insertColumn = qm_.ColumnList_[qm_.ColumnList_.Count - 1].ToUpper();

            insertQuery_SB.AppendLine(upperItem_insertColumn + " )\" +");
