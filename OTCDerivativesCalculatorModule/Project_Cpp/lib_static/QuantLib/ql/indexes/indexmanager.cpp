...
namespace QuantLib {
	...
	//void IndexManager::addLogLog(const std::string& log) {
 //       
 //   }

	void IndexManager::setPastDates(const std::vector<Date>& dates) {
        this->dates_ = dates;
    }

	std::vector<Date> IndexManager::getPastDates(const Date& start,const Date& endDate,bool endDateInclude)
	{
        
		std::vector<Date> d;
		for ( Size i=0;i<this->dates_.size();i++)
		{
			if ( start <= this->dates_[i] &&  this->dates_[i] < endDate )
			{
				d.push_back(this->dates_[i]);
			}
		}

		if ( endDateInclude && this->dates_.back() == endDate )
			d.push_back(endDate);
		
		return d;
    }

	//void IndexManager::setFirstDate(const Date& date)
	//{
	//	this->firstDate_ = date;
	//}

	//Date IndexManager::getFirstDate()
	//{
