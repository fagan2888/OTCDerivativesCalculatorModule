...
namespace QuantLib {
    ...
    class IndexManager : public Singleton<IndexManager> {
      ...
      public:
		...
		void setPastDates(const std::vector<Date>& dates);
		std::vector<Date> getPastDates(const Date& start,const Date& endDate,bool endDateInclude);

		//void setFirstDate(const Date& date);
		//Date getFirstDate();
      ...
      private:
		...
		// 여기에 setting되는 date를 기본으로 과거 path grid를 만들어냄
