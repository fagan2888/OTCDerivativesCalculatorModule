...
class ELSGeneralPricer {
	  ...
	  protected:
		  ...
		  bool checkEarlyRedem(std::valarray<double>& sumdZ,Size i) const;
		  bool checkKI(std::valarray<double>& sumdZ,Size i) const;
		  Real SumdZMin(std::valarray<double>& sumdZ) const;
		  Real slopePayoff(std::valarray<double>& sumdZ) const; //마지막 payoff
