...
namespace QuantLib {
  ...
  //                  Spread spread = 0.0,
		...
		//			bool accrual= false);
		MonteCoupon(Real nominal,			
					const Date& startDate,
                    const Date& endDate,
					Natural fixingDays,
					const DayCounter& dayCounter,
					std::vector<boost::shared_ptr<Index>>& index = std::vector<boost::shared_ptr<Index>>(),
					Real gearing = 1.0,
                    Spread spread = 0.0,
					Real capRate = 1.0,
					Real floorRate = 0.0,
		...
		Real expectedArrAmount(const Array& preRate,const Array& refRate) const { return expectedArrRate(preRate,refRate) * accrualPeriod() * nominal(); }
		void update() { notifyObservers(); }
		DayCounter dayCounter() const{return dayCounter_;};
		
		//inspector
		virtual Rate expectedRate(const Array& preRate,const Array& refRate) const = 0;
		virtual Rate expectedArrRate(const Array& preRate,const Array& refRate) const 
					{return expectedRate(preRate,refRate);}
