...
inline bool ELSGeneralPricer::checkEarlyRedem(Array& SumdZ,Size i) const {
	bool redem=true;
...
inline bool ELSGeneralPricer::checkKI(Array& SumdZ,Size i) const {
bool KI=KIflag_;
	for(Size asset=0;asset<numberOfAssets_;++asset){
...
inline Real ELSGeneralPricer::SumdZMin(Array& SumdZ) const {
	Real min=SumdZ[0];
...
inline Real ELSGeneralPricer::slopePayoff(Array& SumdZ) const {
	Real min=SumdZ[0];
	Size numSumdZ=0;
	for(Size asset=1;asset<numberOfAssets_;++asset){
		if(min>SumdZ[asset]){
						...
						for(Size jj=0;jj<jmax;++jj){
							...
							for(Size assetRow=0;assetRow<numberOfAssets_;++assetRow){
								...
								for(Size assetCol=0;assetCol<assetRow+1;++assetCol){
									...
									randZ+=bridgePath[position+assetCol]*chol_[assetRow][assetCol];
