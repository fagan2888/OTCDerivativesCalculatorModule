...
EventCheck::EventCheck(std::string thisEventSymbolName,
				   ...
				   const std::vector<boost::shared_ptr<EventCheck>>& subEventchecks )
...
void EventCheck::gridInitialize(const TimeGrid& timeGrid , 
...
void EventCheck::addSymbolEvent(const std::string& symbolName){
	thisStoredSymbolEvent_.push_back(symbolName);
}
//일괄적으로 setting함
void EventCheck::defineConst(const std::string varStr,Real constValue){
...
//일괄적으로 setting함
void EventCheck::defineBaseIndex(const std::string varStr){
...
//void EventCheck::defineVar(const std::string varStr){
//}


void EventCheck::calculate(const MultiPath& multiPath){

	if(!isCalFlag_){
		for(Size k=0;k<eventDatesPositionNum_;++k){ //k번 돌도록 하였음. daily로 오는경우
			this->setBaseIndexValue(multiPath,eventDatesPosition_[k]);
			
			for(Size i=0;i<thisStoredSymbolIndex_.size() ;++i){
				//symbolValue_[i]=indexM_Event_->calValue(multiPath);
				/*
				if(indexM_Event_[i]->average()){
					double averValue = 0;
					Size ardpNum = indexM_Event_[i]->averRelativeDatesPosition().size();
					
					for(Size z=0; z < ardpNum ; ++z ){ //그냥 pastIndex에 거꾸로 넣으면 대지..

						//sub가 딴데로 들어갈때에.. 절대값으로 해놨는데
						setBaseIndexValue(multiPath,eventDatesPosition_[k]-averRelativeDatesPosition_[z]);

						for(Size j=0;j<=i;++j){
							//symbolValue_[j] = indexM_Event_[j]->calValue(multiPath,eventDatesPosition_[z]); 
							indexM_Event_[j]->calValue(multiPath,eventDatesPosition_[k]-averRelativeDatesPosition_[z]); 
						}
						averValue += *(indexM_EventSymbolValue_[i]) / static_cast<double>(ardpNum); 
					}
					
					*(indexM_EventSymbolValue_[i]) = averValue; //이걸


					// setBaseIndexValue(multiPath,0); //과거로 setting된 것들을 다시 돌려놈. for 다음 indexTran을 위하여.
			...
			if(functionEventJointOccFlag_){
				for(Size i=0;i<subEventNum_;++i){
					subEventchecks_[i]->calculate(multiPath); //뭐든 필요하면 계산은 먼저 시키고, 나중에 한번일어난걸 가져오던 다른걸 가져오던 (이건 클래스로 갈지 생각해야함) 해야함.
...
Real EventCheck::payoff(const MultiPath& multiPath){
...
//eventCalFunction
bool EventCheck::functionEventsJoint_Cal(){
	bool tf;
...
bool EventCheck::SubEventJoint_Cal(){
	bool tf;
...
bool EventCheck::funcAndSubJointEvent_Cal(){
	bool tf;
...
bool EventCheck::timeEventJoint_Cal(Size startDatePosition,Size refDatePosition){
	bool tf;
...
Size EventCheck::payoffIndexLocation() const{return payoffManager_->payoffDatesPosition();}

void EventCheck::setBaseIndexValue(const MultiPath& multiPath,int datesPosition){
	for(Size i=0;i<baseIndex_.size();++i){
		*baseIndexValue_[i]=multiPath[i][datesPosition];
	}
}

void EventCheck::reset(){
	bool tf = false;
		this->isCalFlag_=tf;
