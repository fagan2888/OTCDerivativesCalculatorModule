...
boost::shared_ptr<ELSGeneralPricer> MCELSGeneralEngine::pathPricer() const{
   ...
   /*boost::shared_ptr<GeneralizedBlackScholesProcess> process =
	...
	QL_REQUIRE(numAssets == initialValues.size(), "Index Number must be same ProcessSize");
	...
	for(Size i=0;i<numAssets;++i){
		...
		drift[i]= process_->process(i)->drift(times[0],1.0); //discountTS_->zeroRate(times.back(),Compounded,Annual,false);
