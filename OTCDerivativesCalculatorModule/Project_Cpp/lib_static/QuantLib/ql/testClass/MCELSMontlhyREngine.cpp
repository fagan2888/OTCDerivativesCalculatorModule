...
MCELSMonthlyREngine::MCELSMonthlyREngine(
...
TimeGrid MCELSMonthlyREngine::timeGrid() const {
...
boost::shared_ptr<ELSGeneralMonthlyRPricer> MCELSMonthlyREngine::pathPricer() const{
   ...
   /*boost::shared_ptr<GeneralizedBlackScholesProcess> process =
	...
	Real KILossCoupon = this->arguments_.KILossCoupon;
	Size pastKICount = this->arguments_.pastKICount;
	std::vector<Date> remainfixings;
	...
	Size m=0;
	if(checkKIType==DailyKI){m=3;}
	else if(checkKIType==ContiKI){m=4;}
	else{m=2;}
	...
	std::vector<bool> earlyRedemtimePositions(numberOfTimes);
	
	for(Size i=0;i<numberOfTimes;++i){
		if(earlyExValues[i] > 0.0){
			earlyRedemtimePositions[i]=true;
		}
	}
...
//			std::cout << " --------------------------------------------------------------"<< std::endl;
	...
	return boost::shared_ptr<ELSGeneralMonthlyRPricer>(
					new ELSGeneralMonthlyRPricer(times,process_,x0,discountFactors,
											 earlyRedemtimePositions,
											 KIRedemPayoffs,earlyExValues_Chg,KIbarrierValues_Chg,
											 redemCoupon,KILossCoupon,
											 pastFixingNum,pastKICount,
											 seed_,additionalStats_,checkKIType,KIflag));
}
