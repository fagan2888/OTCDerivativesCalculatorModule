...
Real ELSGeneralPricer::values(Size simulNum) const {
	...
	for(Size i=0;i<numberOfAssets_;++i){
		...
		//x0_[i] = x0_def_[i] + 0.00498754151103897; // ln(1.01) = 0.00995033085316809; , ln(1.01*St/S0) = ln(1.1) + ln(St/S0)
		...
		//x0_[i] = x0_def_[i]  -0.00501254182354429; // ln(0.99) = -0.0100503358535015; , ln(0.99*St/S0) = ln(1.1) + ln(St/S0)
		...
		for(Size j=0 ;j<simulNum;++j){
			...
			for (Size i = 0; i <numberOfTimes; ++i) {
				...
				if (checkEarlyRedem(SumdZ,i))	{
					...
					earlyExflag=true;
					randSeqIter=randSeqIter + numberOfAssets_ * (numberOfTimes - i);
					break;
				}
