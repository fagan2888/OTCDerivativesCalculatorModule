...
#include <ql/indexes/indexType.hpp>
    ...
    class StochasticProcess : public Observer, public Observable {
      public:
        ...
        */
		...
		// This method use saved Data
		virtual Disposable<Array> evolve(Time t0, const Array& x0,
                                         Time dt, const Array& dw,Size z) const;
    ...
    class StochasticProcess1D : public StochasticProcess {
      public:
        ...
        */
		...
		// This method use saved Data
		virtual Real evolve(Time t0, Real x0, Time dt, Real dw,Size z) const;
		
		// 속도를 위해 수정되었음.
		//virtual preCalculate(const TimeGrid& timeGrid) const{};

        /*! applies a change to the asset value. By default, it
            returns \f$ x + \Delta x \f$.
        */
        virtual Real apply(Real x0, Real dx) const;
		// Store Method for PreCalulation. ex) forwardRate
		virtual void setPreCalculation(const TimeGrid& timeGrid) const{};
		virtual Period tenor() const{return Period();};

		// 이자율을 바꾸기위해서 만들어졌음. 2012-02-17
		virtual void indexAdjustment(std::vector<Array>& path,std::vector<Real>& randz,IndexType type) const{};
		//virtual Real evolveSaved(Real x0,Real z,Size i) const{return x0;};

		// basePrice가 여기 박혀있는게 맞는거 같음 ( 최초 기준가 ) 2013-11-28
		virtual Real basePrice() const { return this->basePrice_; }
		virtual void setBasePrice(Real value) const { this->basePrice_ = value; }

		// code 구분 2013-11-28
		virtual std::string code() const { return this->code_; }
		virtual void setCode(std::string value) const { this->code_ = value; }
