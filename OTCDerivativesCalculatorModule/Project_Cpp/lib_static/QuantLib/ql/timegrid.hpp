    ...
    class TimeGrid {
      public:
        ...
        */
        ...
        : mandatoryTimes_(begin, end), mandatoryDates_(dates) {
            std::sort(mandatoryTimes_.begin(),mandatoryTimes_.end());
			std::sort(mandatoryDates_.begin(),mandatoryDates_.end());

			QL_REQUIRE(mandatoryTimes_.size() == mandatoryDates_.size(),
            "fixingTimes size is not same to fixingDates size");
        ...
        */
        ...
        //@}
		//Ãß°¡µÊ 2013-12-18
		Date dateAt(Size i) const { return mandatoryDates_[i]; }
		Time dateTime(const Date& date) const { return times_.at(this->dateIndex(date)); }
		Size dateIndex(const Date& date) const;
		std::vector<Date> dates() const { return this->mandatoryDates_; }
		//Ãß°¡µÊ 2014-11-11
		Size todayIndex() const;
		Time todayTime() const;

      private:
        std::vector<Time> times_;
        std::vector<Time> dt_;
        std::vector<Time> mandatoryTimes_;
		std::vector<Date> mandatoryDates_;
    };
