...
namespace QuantLib {
    ...
    void MidPointCdsEngine::calculate() const {
		...
		Size legSize = arguments_.leg.size();

		results_.survialProb.resize(legSize);
		results_.defaultProb.resize(legSize);
		results_.expectedCouponLegNPV.resize(legSize);
		results_.expectedprotectionLegNPV.resize(legSize);
		results_.paymentDate.resize(legSize);
		results_.discountFactorAtPayment.resize(legSize);
        ...
        for (Size i=0; i<arguments_.leg.size(); ++i) {
			...
			//2013-10-7 값 검정을 위해 추가함
			results_.paymentDate[i] = paymentDate;
			results_.discountFactorAtPayment[i] = discountCurve_->discount(paymentDate);
            ...
            Probability P = probability_->defaultProbability(
                                                ...
                                                endDate);
			//2013-10-7 값 검정을 위해 추가함
			results_.survialProb[i] = S;
			results_.defaultProb[i] = P;

            // on one side, we add the fixed rate payments in case of
            // survival...
			Real legNPV = S * coupon->amount() *
                discountCurve_->discount(paymentDate);

			results_.couponLegNPV += legNPV;
			results_.expectedCouponLegNPV[i] = legNPV;
            ...
            if (arguments_.settlesAccrual) {
                if (arguments_.paysAtDefaultTime) {
					Real paysAtDefaultValue = P * coupon->accruedAmount(defaultDate) *
                        discountCurve_->discount(defaultDate);
					
					//2013-10-7 값 검정을 위해 추가함
					results_.couponLegNPV += paysAtDefaultValue;
					results_.expectedCouponLegNPV[i] +=  paysAtDefaultValue;
					...
					Real paysAtPaymentValue = P * coupon->amount() *
                        discountCurve_->discount(paymentDate);
					
					//2013-10-7 값 검정을 위해 추가함
					results_.couponLegNPV += paysAtPaymentValue;
					results_.expectedCouponLegNPV[i] += paysAtPaymentValue;
            ...
            Real claim = arguments_.claim->amount(defaultDate,
                                                  arguments_.notional,
                                                  recoveryRate_);
			Real defaultLegNPVValue = 0.0;

            if (arguments_.paysAtDefaultTime) {
				defaultLegNPVValue = P * claim * discountCurve_->discount(defaultDate);

				results_.defaultLegNPV += defaultLegNPVValue;
				results_.expectedprotectionLegNPV[i] = defaultLegNPVValue;

                /*results_.defaultLegNPV +=
                    P * claim * discountCurve_->discount(defaultDate);*/
            } else {
				defaultLegNPVValue = P * claim * discountCurve_->discount(paymentDate);
