...
namespace QuantLib {
	...
	//! Constant Payoff  2012-08-28 DLS인가 관련 , Upper Barrier 쳤을때 고정 수익을 위해 만들었음.
    class ConstantPayoff : public Payoff {
      public:
        //! \name Payoff interface
        //@{
		ConstantPayoff(Real rebate) : rebate_(rebate){}
        std::string name() const{ return "ConstantPayoff";}
        std::string description() const;
        Real operator()(Real price) const;
        virtual void accept(AcyclicVisitor&);
	  private:
	...
	class IdentityPayoff : public Payoff {
      public:
        //! \name Payoff interface
        //@{
		IdentityPayoff(){}
        std::string name() const{ return "IdentityPayoff";}
        std::string description() const;
        Real operator()(Real price) const;
        virtual void accept(AcyclicVisitor&);
    ...
    class PlainVanillaPayoff : public StrikedTypePayoff {
      public:
		...
		PlainVanillaPayoff(Option::Type type,
                           Real strike,Real partiRate = 1.0)
        : StrikedTypePayoff(type, strike) , partiRate_(partiRate) {}
	...
	//! max(return - strike_(eq. 1.0 ), 0.0) 형식의 payoff // initial(기준가)을 가지고 있어야함.
	// strike_ 가 보통 1.0(default) 이 들어감. 
	 class ReturnStrikePayoff : public StrikedTypePayoff {
      public:
        ReturnStrikePayoff(Option::Type type,
                               Real initial,Real percentStrike = 1.0)
        : StrikedTypePayoff(type, percentStrike) , initial_(initial) {}
        //! \name Payoff interface
        //@{
        std::string name() const { return "ReturnStrikePayoff";}
        Real operator()(Real price) const;
        virtual void accept(AcyclicVisitor&);
	  private:
		Real initial_;
