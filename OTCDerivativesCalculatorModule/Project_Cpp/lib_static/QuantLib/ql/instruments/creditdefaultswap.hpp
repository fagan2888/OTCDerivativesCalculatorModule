    ...
    */
    ...
    class CreditDefaultSwap::results : public Instrument::results {
      public:
		...
		//2013-10-07 Credit 값 검정을 위해 추가함
		std::vector<Real> survialProb;
		std::vector<Real> defaultProb;

		std::vector<Real> expectedCouponLegNPV;
		std::vector<Real> expectedprotectionLegNPV;
		std::vector<Date> paymentDate;
		std::vector<Real> discountFactorAtPayment;
