...
#include <ql/instruments/warrantoption.hpp>
...
namespace QuantLib {
    ...
    WarrantOption::WarrantOption(const Date& issueDate,
 							  const Date& maturityDate,
							  const boost::shared_ptr<StockIndex>& stockIndex,
							  Real initialStrike,
							  const Schedule& exerciseTenor,								 
							  const Schedule& refixingTenor=Schedule(),
							  Real refixingRate=0.0)
    : issueDate_(issueDate),maturityDate_(maturityDate),stockIndex_(stockIndex),
	  initialStrike_(initialStrike),exerciseTenor_(exerciseTenor),refixingTenor_(refixingTenor),
	  refixingRate_(refixingRate) {}


    Volatility WarrantOption::impliedVolatility(
             Real targetValue,
