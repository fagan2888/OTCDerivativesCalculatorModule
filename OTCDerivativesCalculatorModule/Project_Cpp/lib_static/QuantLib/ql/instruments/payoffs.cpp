...
namespace QuantLib {
    ...
    std::string ConstantPayoff::description() const {
        return name();
    }

    Real ConstantPayoff::operator()(Real) const {
        return rebate_;
    }

    void ConstantPayoff::accept(AcyclicVisitor& v) {
        Visitor<ConstantPayoff>* v1 =
            dynamic_cast<Visitor<ConstantPayoff>*>(&v);
        if (v1 != 0)
            v1->visit(*this);
        else
            Payoff::accept(v);
    ...
    std::string IdentityPayoff::description() const {
        return name();
    }

    Real IdentityPayoff::operator()(Real price) const {
        return price;
    }

    void IdentityPayoff::accept(AcyclicVisitor& v) {
        Visitor<IdentityPayoff>* v1 =
            dynamic_cast<Visitor<IdentityPayoff>*>(&v);
        if (v1 != 0)
            v1->visit(*this);
        else
            Payoff::accept(v);
    ...
    Real PlainVanillaPayoff::operator()(Real price) const {
        switch (type_) {
          case Option::Call:
            return partiRate_*std::max<Real>(price-strike_,0.0);
          case Option::Put:
            return partiRate_*std::max<Real>(strike_-price,0.0);
          default:
	...
	Real ReturnStrikePayoff::operator()(Real price) const {
        switch (type_) {
          case Option::Call:
            return std::max<Real>((price/initial_) - strike_,0.0);
          case Option::Put:
            return std::max<Real>(strike_ - (price/initial_),0.0);
          default:
            QL_FAIL("unknown/illegal option type");
        }
    }

    void ReturnStrikePayoff::accept(AcyclicVisitor& v) {
        Visitor<ReturnStrikePayoff>* v1 =
            dynamic_cast<Visitor<ReturnStrikePayoff>*>(&v);
        if (v1 != 0)
            v1->visit(*this);
        else
            Payoff::accept(v);
