    ...
    */
    ...
    class TimeSeries {
        ...
        */
        ...
        T operator[](const Date& d) const {
            if (values_.find(d) != values_.end())
				...
				QL_REQUIRE(true,"Data does not exist at " << d);
                return Null<T>();
        }
        T& operator[](const Date& d) {
            if (values_.find(d) == values_.end())
                //values_[d] = Null<T>(); // 수정함. 2012-05-25 Null<T> 에 큰값이 들어가서, Exception을 내도록 수정
				QL_REQUIRE(true,"Data does not exist at " << d);
            return values_[d];
        }
        //@}
      ...
      public:
		...
		const_iterator find(const Date&) const;
        //! returns the dates for which historical data exist
        std::vector<Date> dates() const;
		//! 2012-05-09 에 추가한 method KI 검사 속도를 위해 하나 만들음
		//! returns the dates in interval for which historical data exist 
		std::vector<Date> dates(const Date&,const Date&) const;
        //! returns the historical data
        std::vector<T> values() const;
		//! 2012-05-09 에 추가한 method KI 검사 속도를 위해 하나 만들음
		//! returns the historical data in interval
		std::vector<T> values(const Date&,const Date&) const;
	...
	template <class T, class C>
    inline typename TimeSeries<T,C>::const_iterator
    TimeSeries<T,C>::find(const Date& d) const{
        const_iterator i = values_.find(d);
        if (i == values_.end()) {
            values_[d] = Null<T>();
            i = values_.find(d);
	...
	template <class T, class C>
    std::vector<Date> TimeSeries<T,C>::dates(const Date& beg,const Date& end) const{
        std::vector<Date> v;
        v.reserve(size());
        std::transform(find(beg), find(end), std::back_inserter(v),
                       TimeSeries<T,C>::get_time);
        return v;
    }
	...
	template <class T, class C>
    std::vector<T> TimeSeries<T,C>::values(const Date& beg,const Date& end) const{
        std::vector<T> v;
        v.reserve(size());
        std::transform(find(beg), find(end), std::back_inserter(v),
                       TimeSeries<T,C>::get_value);
