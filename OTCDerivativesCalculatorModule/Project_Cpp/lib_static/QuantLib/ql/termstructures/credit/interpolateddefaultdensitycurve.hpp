...
namespace QuantLib {
    ...
    InterpolatedDefaultDensityCurve<T>::InterpolatedDefaultDensityCurve(
                                    ...
                                    const std::vector<Date>& jumpDates,
                                    const T& interpolator)
    //: DefaultDensityStructure(dates.front(), calendar, dayCounter, jumps, jumpDates),// Tag :: TS_ReferenceDate_Change
	: DefaultDensityStructure(Settings::instance().evaluationDate(), calendar, dayCounter, jumps, jumpDates),
    ...
    InterpolatedDefaultDensityCurve<T>::InterpolatedDefaultDensityCurve(
            ...
            const Calendar& calendar,
            const T& interpolator)
    //: DefaultDensityStructure(dates.front(), calendar, dayCounter),// Tag :: TS_ReferenceDate_Change
	: DefaultDensityStructure(Settings::instance().evaluationDate(), calendar, dayCounter),
      InterpolatedCurve<T>(std::vector<Time>(), densities, interpolator),
      dates_(dates)
    {
        initialize(dates, densities, dayCounter);
