...
namespace QuantLib{
	...
	class CopulaHazardRate : public InterpolatedDefaultDensityCurve<Interpolator> {
	public:
		typedef boost::shared_ptr<InterpolatedDefaultDensityCurve<Interpolator> > base_curve;
		CopulaHazardRate(
				...
				: InterpolatedDefaultDensityCurve<Interpolator>(probabilities[0]->referenceDate(),probabilities[0]->dayCounter()),
    ...
    void CopulaHazardRate<TraitProb,Interpolator>::initialize()
    {
		...
		calculate();
		InterpolatedDefaultDensityCurve<Interpolator>::initialize();
    }
