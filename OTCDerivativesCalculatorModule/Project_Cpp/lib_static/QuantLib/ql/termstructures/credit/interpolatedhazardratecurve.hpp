...
namespace QuantLib {
    ...
    InterpolatedHazardRateCurve<T>::InterpolatedHazardRateCurve(
                                    ...
                                    const std::vector<Date>& jumpDates,
                                    const T& interpolator)
    //: HazardRateStructure(dates.front(), calendar, dayCounter, jumps, jumpDates), // Tag :: TS_ReferenceDate_Change
	: HazardRateStructure(Settings::instance().evaluationDate(), calendar, dayCounter, jumps, jumpDates),
    ...
    InterpolatedHazardRateCurve<T>::InterpolatedHazardRateCurve(
            ...
            const Calendar& calendar,
            const T& interpolator)
    //: HazardRateStructure(dates.front(), calendar, dayCounter),  // Tag :: TS_ReferenceDate_Change
	: HazardRateStructure(Settings::instance().evaluationDate(), calendar, dayCounter),
      InterpolatedCurve<T>(std::vector<Time>(), hazardRates, interpolator),
      dates_(dates)
    {
        initialize();
    ...
    void InterpolatedHazardRateCurve<T>::initialize()
    {
		...
		// 2012-05-25 data_가 하나씩 밀려서 mapping 되어 수정함.//3Day보다 작으면 통합함.
