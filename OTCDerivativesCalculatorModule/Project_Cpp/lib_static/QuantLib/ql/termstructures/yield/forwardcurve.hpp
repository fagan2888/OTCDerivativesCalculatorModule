...
namespace QuantLib {
    ...
    InterpolatedForwardCurve<T>::InterpolatedForwardCurve(
                                    ...
                                    const std::vector<Date>& jumpDates,
                                    const T& interpolator)
    //: ForwardRateStructure(dates.front(), calendar, dayCounter, jumps, jumpDates), // Tag :: TS_ReferenceDate_Change
	: ForwardRateStructure(Settings::instance().evaluationDate(), calendar, dayCounter, jumps, jumpDates),
    ...
    InterpolatedForwardCurve<T>::InterpolatedForwardCurve(
            ...
            const Calendar& calendar,
            const T& interpolator)
    //: ForwardRateStructure(dates.front(), calendar, dayCounter), // Tag :: TS_ReferenceDate_Change
	: ForwardRateStructure(Settings::instance().evaluationDate(), calendar, dayCounter), 
      InterpolatedCurve<T>(std::vector<Time>(), forwards, interpolator),
      dates_(dates)
    {
        initialize();
