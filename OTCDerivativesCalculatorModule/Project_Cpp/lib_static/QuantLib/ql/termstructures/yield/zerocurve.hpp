...
namespace QuantLib {
    ...
    InterpolatedZeroCurve<T>::InterpolatedZeroCurve(
                                    ...
                                    const std::vector<Date>& jumpDates,
                                    const T& interpolator)
    //: ZeroYieldStructure(dates.front(), calendar, dayCounter, jumps, jumpDates), // Tag :: TS_ReferenceDate_Change
	: ZeroYieldStructure(Settings::instance().evaluationDate(), calendar, dayCounter, jumps, jumpDates),
    ...
    InterpolatedZeroCurve<T>::InterpolatedZeroCurve(
                                               ...
                                               const Calendar& calendar,
                                               const T& interpolator)
    //: ZeroYieldStructure(dates.front(), calendar, dayCounter), // Tag :: TS_ReferenceDate_Change
	: ZeroYieldStructure(Settings::instance().evaluationDate(), calendar, dayCounter),
      InterpolatedCurve<T>(std::vector<Time>(), yields, interpolator),
      dates_(dates)
    {
        initialize();
