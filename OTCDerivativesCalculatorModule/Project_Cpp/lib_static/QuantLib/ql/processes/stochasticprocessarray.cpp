...
namespace QuantLib {
	...
	//속도 향상을 위한 인터페이스
	Disposable<Array> StochasticProcessArray::evolve(
                  Time t0, const Array& x0, Time dt, const Array& dw,Size z) const {
        const Array dz = sqrtCorrelation_ * dw;

        Array tmp(size());
        for (Size i=0; i<size(); ++i)
            tmp[i] = processes_[i]->evolve(t0, x0[i], dt, dz[i],z);
        return tmp;
    }

	Disposable<Array> StochasticProcessArray::basePrices() const {
        Array tmp(size());
        for (Size i=0; i<size(); ++i)
            tmp[i] = processes_[i]->basePrice();
        return tmp;
    }
