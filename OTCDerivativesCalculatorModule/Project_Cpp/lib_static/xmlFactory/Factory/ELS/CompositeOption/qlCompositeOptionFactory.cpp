#include "qlCompositeOptionFactory.hpp"
...
qlCompositeOptionFactory::qlCompositeOptionFactory()
{
}

void qlCompositeOptionFactory::calculate()
{
	this->factoryCompositeOption();
...
void qlCompositeOptionFactory::calculate(const std::string& instInfoStr,
...
boost::shared_ptr<MonteEngine> qlCompositeOptionFactory::engine()
{
...
void qlCompositeOptionFactory::factoryCompositeOption()
{
	...
	boost::shared_ptr<FpmlSerialized::Excel_compositeOption> xml_compOption = xml_interface->getExcel_compositeOption();

	DayCounter daycounter = Actual365Fixed();
	Calendar calendar = SouthKorea(); // ?

	boost::shared_ptr<FpmlSerialized::Excel_noteInfo> xml_noteInfo = xml_compOption->getExcel_issueInfo()->getExcel_noteInfo();
	...
	std::vector<boost::shared_ptr<FpmlSerialized::Excel_compositeOption_subtype>> xml_compOptionSubList = xml_compOption->getExcel_compositeOption_subtype();
	...
	for(Size i = 0 ; i<xml_compOptionSubList.size(); ++i )
	{
		const boost::shared_ptr<EventTriggerInfo>& ql_eti
			= qetif.compOptionSubType(xml_compOptionSubList[i]);
	...
	Real notionalPayment = xml_compOption->getNotionalMaturityPayment()->DValue();

	Date notionalPayment_PayoffDate = xml_compOption->getPayoffDate()->dateValue();
