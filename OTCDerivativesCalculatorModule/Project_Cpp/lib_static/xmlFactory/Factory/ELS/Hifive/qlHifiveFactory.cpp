...
qlHifiveFactory::qlHifiveFactory()
{
}

void qlHifiveFactory::calculate()
{
	this->factoryHifive();
...
void qlHifiveFactory::calculate(const std::string& instInfoStr,
...
boost::shared_ptr<MonteEngine> qlHifiveFactory::engine()
{
	...
	boost::shared_ptr<FpmlSerialized::Excel_standardHifivePara> xml_standardHifive_para = xml_para->getExcel_standardHifivePara();
	

	boost::shared_ptr<FpmlSerialized::Excel_underlyingCalcInfo_para> xml_excelUnderyingCalcInfo_para = xml_standardHifive_para->getExcel_underlyingCalcInfo_para();
	...
	boost::shared_ptr<FpmlSerialized::Excel_discountCurve_para> xml_discount = xml_standardHifive_para->getExcel_discountCurve_para();

	std::vector<boost::shared_ptr<YieldTermStructure>> yieldTSList
		= ql_tysf.e_yieldTermStructureList(xml_discount);

	Size simulNum = xml_standardHifive_para->getSimulationNum()->IValue();
...
void qlHifiveFactory::factoryHifive()
{
	...
	boost::shared_ptr<FpmlSerialized::Excel_noteInfo> xml_noteInfo = xml_hifive->getExcel_issueInfo()->getExcel_noteInfo();
	...
	std::vector<boost::shared_ptr<FpmlSerialized::Excel_schedule>> xml_scheduleList = xml_hifive->getExcel_scheduleList()->getExcel_schedule();
	...
	const boost::shared_ptr<EventTriggerInfo>& ql_kiEti
			= qetif.excel_kiEventCalculation(xml_hifive->getExcel_kiEventCalculation());
	...
	//this->pricer_ = boost::shared_ptr<HifivePricer>( 
