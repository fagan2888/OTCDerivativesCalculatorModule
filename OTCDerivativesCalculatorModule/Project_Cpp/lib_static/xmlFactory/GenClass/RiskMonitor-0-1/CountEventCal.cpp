...
CountEventCal::CountEventCal(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
...
boost::shared_ptr<XsdTypeInteger> CountEventCal::getNum()
{
   if(!this->numIsNull_){
        return this->num_;
   }else
...
boost::shared_ptr<EventCalculationList> CountEventCal::getEventCalculationList()
{
...
boost::shared_ptr<XsdTypeBoolean> CountEventCal::getComplement()
{
