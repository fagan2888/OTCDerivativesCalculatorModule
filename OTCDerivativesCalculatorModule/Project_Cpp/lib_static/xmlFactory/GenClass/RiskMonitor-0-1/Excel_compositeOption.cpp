// Excel_compositeOption.cpp 
#include "Excel_compositeOption.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
Excel_compositeOption::Excel_compositeOption(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   //excel_compositeOption_subtypeNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* excel_compositeOption_subtypeNode = xmlNode->FirstChildElement("excel_compositeOption_subtype");

   if(excel_compositeOption_subtypeNode){excel_compositeOption_subtypeIsNull_ = false;}
   else{excel_compositeOption_subtypeIsNull_ = true;}

   if(excel_compositeOption_subtypeNode)
   {
      for(excel_compositeOption_subtypeNode; excel_compositeOption_subtypeNode; excel_compositeOption_subtypeNode = excel_compositeOption_subtypeNode->NextSiblingElement("excel_compositeOption_subtype")){
          #ifdef ConsolePrint
              FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- excel_compositeOption_subtypeNode , address : " << excel_compositeOption_subtypeNode << std::endl;
          #endif
          excel_compositeOption_subtype_.push_back(boost::shared_ptr<Excel_compositeOption_subtype>(new Excel_compositeOption_subtype(excel_compositeOption_subtypeNode)));
      }
   }
   else {
       #ifdef ConsolePrint
...
boost::shared_ptr<Excel_issueInfo> Excel_compositeOption::getExcel_issueInfo()
{
...
boost::shared_ptr<Excel_underlyingCalcInfo> Excel_compositeOption::getExcel_underlyingCalcInfo()
{
...
boost::shared_ptr<XsdTypeToken> Excel_compositeOption::getExerciseDate()
{
...
boost::shared_ptr<XsdTypeToken> Excel_compositeOption::getPayoffDate()
{
...
boost::shared_ptr<XsdTypeToken> Excel_compositeOption::getNotionalMaturityPayment()
{
...
std::vector<boost::shared_ptr<Excel_compositeOption_subtype>> Excel_compositeOption::getExcel_compositeOption_subtype()
{
   if(!this->excel_compositeOption_subtypeIsNull_){
        return this->excel_compositeOption_subtype_;
   }else
   {
      QL_FAIL("null Ptr");
      return std::vector<boost::shared_ptr<Excel_compositeOption_subtype>>();
   }
}
}
