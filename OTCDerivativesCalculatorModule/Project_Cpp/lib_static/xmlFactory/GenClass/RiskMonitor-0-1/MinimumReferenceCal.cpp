...
MinimumReferenceCal::MinimumReferenceCal(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
...
boost::shared_ptr<XsdTypeDouble> MinimumReferenceCal::getNth()
{
   if(!this->nthIsNull_){
        return this->nth_;
   }else
...
std::vector<boost::shared_ptr<XsdTypeToken>> MinimumReferenceCal::getSelectedUnderName()
{
...
boost::shared_ptr<FixingDateInfo> MinimumReferenceCal::getFixingDateInfo()
{
