...
#include <ql/experimental/finitedifferences/FdmStructuredProductEngine.hpp>
#include <ql/experimental/finitedifferences/fdmbermudanstepcondition.hpp>

namespace QuantLib {

    FdmStructuredProductEngine::FdmStructuredProductEngine(
    ...
    void FdmStructuredProductEngine::calculate() const {
        // 1. Layout
        std::vector<Size> dim;
		...
		boost::shared_ptr<FdmContingentPayoffComposite>& payoffComposite
						= arguments_.payoffComposite;
		
		payoffComposite->build(mesher);
        ...
        // 6. Solver
        boost::shared_ptr<Fdm2dParallelSolver> solver(
                new Fdm2dParallelSolver(
                             Handle<GeneralizedBlackScholesProcess>(p1_),
                             Handle<GeneralizedBlackScholesProcess>(p2_),
                             correlation_,
                             mesher, boundaries, payoffComposite, 
