...
namespace QuantLib {
    ...
    class Fdm2dParallelSolver : public LazyObject {
      public:
        Fdm2dParallelSolver(
      ...
      private:
		...
		const boost::shared_ptr<FdmContingentPayoffComposite> contingentPayoff_,
		...
		std::valarray<Array> initialValues_;

        mutable Matrix resultValues_;
        mutable boost::shared_ptr<BicubicSpline> interpolation_;

		FiniteDifferenceModel<ParallelEvolverOverWrap<ImplicitEulerScheme>> parallel_model_;
