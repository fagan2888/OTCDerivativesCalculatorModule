...
PrePayment::PrePayment(TiXmlNode* xmlNode)
: PaymentBase(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(prePaymentNode)
   {
      if(prePaymentNode->Attribute("href") || prePaymentNode->Attribute("id"))
      {
          if(prePaymentNode->Attribute("id"))
          {
             ...
             prePayment_ = boost::shared_ptr<XsdTypeBoolean>(new XsdTypeBoolean(prePaymentNode));
      ...
      else { prePayment_ = boost::shared_ptr<XsdTypeBoolean>(new XsdTypeBoolean(prePaymentNode));}
   }

   //prePaymentAmountNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* prePaymentAmountNode = xmlNode->FirstChildElement("prePaymentAmount");

   if(prePaymentAmountNode){prePaymentAmountIsNull_ = false;}
   else{prePaymentAmountIsNull_ = true;}

   #ifdef ConsolePrint
      FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- prePaymentAmountNode , address : " << prePaymentAmountNode << std::endl;
   #endif
   if(prePaymentAmountNode)
   {
      if(prePaymentAmountNode->Attribute("href") || prePaymentAmountNode->Attribute("id"))
      {
          if(prePaymentAmountNode->Attribute("id"))
          {
             prePaymentAmountIDRef_ = prePaymentAmountNode->Attribute("id");
             prePaymentAmount_ = boost::shared_ptr<NonNegativeMoney>(new NonNegativeMoney(prePaymentAmountNode));
             prePaymentAmount_->setID(prePaymentAmountIDRef_);
             IDManager::instance().setID(prePaymentAmountIDRef_,prePaymentAmount_);
          }
          else if(prePaymentAmountNode->Attribute("href")) { prePaymentAmountIDRef_ = prePaymentAmountNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { prePaymentAmount_ = boost::shared_ptr<NonNegativeMoney>(new NonNegativeMoney(prePaymentAmountNode));}
   }

   //prePaymentDateNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* prePaymentDateNode = xmlNode->FirstChildElement("prePaymentDate");

   if(prePaymentDateNode){prePaymentDateIsNull_ = false;}
   else{prePaymentDateIsNull_ = true;}

   #ifdef ConsolePrint
      FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- prePaymentDateNode , address : " << prePaymentDateNode << std::endl;
   #endif
   if(prePaymentDateNode)
   {
      if(prePaymentDateNode->Attribute("href") || prePaymentDateNode->Attribute("id"))
      {
          if(prePaymentDateNode->Attribute("id"))
          {
             prePaymentDateIDRef_ = prePaymentDateNode->Attribute("id");
             prePaymentDate_ = boost::shared_ptr<AdjustableDate>(new AdjustableDate(prePaymentDateNode));
             prePaymentDate_->setID(prePaymentDateIDRef_);
             IDManager::instance().setID(prePaymentDateIDRef_,prePaymentDate_);
          }
          else if(prePaymentDateNode->Attribute("href")) { prePaymentDateIDRef_ = prePaymentDateNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { prePaymentDate_ = boost::shared_ptr<AdjustableDate>(new AdjustableDate(prePaymentDateNode));}
   }
...
boost::shared_ptr<PartyReference> PrePayment::getPayerPartyReference()
{
...
boost::shared_ptr<AccountReference> PrePayment::getPayerAccountReference()
{
...
boost::shared_ptr<PartyReference> PrePayment::getReceiverPartyReference()
{
...
boost::shared_ptr<AccountReference> PrePayment::getReceiverAccountReference()
{
...
boost::shared_ptr<XsdTypeBoolean> PrePayment::getPrePayment()
{
   if(!this->prePaymentIsNull_){
        if(prePaymentIDRef_ != ""){
             return boost::shared_static_cast<XsdTypeBoolean>(IDManager::instance().getID(prePaymentIDRef_));
        }else{
...
boost::shared_ptr<NonNegativeMoney> PrePayment::getPrePaymentAmount()
{
   if(!this->prePaymentAmountIsNull_){
        if(prePaymentAmountIDRef_ != ""){
             return boost::shared_static_cast<NonNegativeMoney>(IDManager::instance().getID(prePaymentAmountIDRef_));
        }else{
...
boost::shared_ptr<AdjustableDate> PrePayment::getPrePaymentDate()
{
   if(!this->prePaymentDateIsNull_){
        if(prePaymentDateIDRef_ != ""){
             return boost::shared_static_cast<AdjustableDate>(IDManager::instance().getID(prePaymentDateIDRef_));
        }else{
