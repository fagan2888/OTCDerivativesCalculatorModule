// CorrectableRequestMessage.cpp 
#include "CorrectableRequestMessage.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
CorrectableRequestMessage::CorrectableRequestMessage(TiXmlNode* xmlNode)
: RequestMessage(xmlNode)
{
   ...
   if(isCorrectionNode)
   {
      if(isCorrectionNode->Attribute("href") || isCorrectionNode->Attribute("id"))
      {
          if(isCorrectionNode->Attribute("id"))
          {
             isCorrectionIDRef_ = isCorrectionNode->Attribute("id");
             isCorrection_ = boost::shared_ptr<XsdTypeBoolean>(new XsdTypeBoolean(isCorrectionNode));
             isCorrection_->setID(isCorrectionIDRef_);
             IDManager::instance().setID(isCorrectionIDRef_,isCorrection_);
          }
          else if(isCorrectionNode->Attribute("href")) { isCorrectionIDRef_ = isCorrectionNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { isCorrection_ = boost::shared_ptr<XsdTypeBoolean>(new XsdTypeBoolean(isCorrectionNode));}
   }
...
boost::shared_ptr<XsdTypeBoolean> CorrectableRequestMessage::getIsCorrection()
{
   if(!this->isCorrectionIsNull_){
        if(isCorrectionIDRef_ != ""){
             return boost::shared_static_cast<XsdTypeBoolean>(IDManager::instance().getID(isCorrectionIDRef_));
        }else{
...
boost::shared_ptr<CorrelationId> CorrectableRequestMessage::getParentCorrelationId()
{
...
boost::shared_ptr<CorrelationId> CorrectableRequestMessage::getCorrelationId()
{
...
boost::shared_ptr<XsdTypePositiveInteger> CorrectableRequestMessage::getSequenceNumber()
{
...
boost::shared_ptr<OnBehalfOf> CorrectableRequestMessage::getOnBehalfOf()
{
