// NotificationMessageHeader.cpp 
#include "NotificationMessageHeader.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
NotificationMessageHeader::NotificationMessageHeader(TiXmlNode* xmlNode)
: MessageHeader(xmlNode)
{
    ...
    #endif 
   //inReplyToNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* inReplyToNode = xmlNode->FirstChildElement("inReplyTo");

   if(inReplyToNode){inReplyToIsNull_ = false;}
   else{inReplyToIsNull_ = true;}

   #ifdef ConsolePrint
      FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- inReplyToNode , address : " << inReplyToNode << std::endl;
   #endif
   if(inReplyToNode)
   {
      if(inReplyToNode->Attribute("href") || inReplyToNode->Attribute("id"))
      {
          if(inReplyToNode->Attribute("id"))
          {
             inReplyToIDRef_ = inReplyToNode->Attribute("id");
             inReplyTo_ = boost::shared_ptr<MessageId>(new MessageId(inReplyToNode));
             inReplyTo_->setID(inReplyToIDRef_);
             IDManager::instance().setID(inReplyToIDRef_,inReplyTo_);
          }
          else if(inReplyToNode->Attribute("href")) { inReplyToIDRef_ = inReplyToNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { inReplyTo_ = boost::shared_ptr<MessageId>(new MessageId(inReplyToNode));}
   }
...
boost::shared_ptr<MessageId> NotificationMessageHeader::getInReplyTo()
{
   if(!this->inReplyToIsNull_){
        if(inReplyToIDRef_ != ""){
             return boost::shared_static_cast<MessageId>(IDManager::instance().getID(inReplyToIDRef_));
        }else{
...
boost::shared_ptr<MessageAddress> NotificationMessageHeader::getSentBy()
{
...
std::vector<boost::shared_ptr<MessageAddress>> NotificationMessageHeader::getSendTo()
{
...
std::vector<boost::shared_ptr<MessageAddress>> NotificationMessageHeader::getCopyTo()
{
...
boost::shared_ptr<XsdTypeDateTime> NotificationMessageHeader::getCreationTimestamp()
{
...
boost::shared_ptr<XsdTypeDateTime> NotificationMessageHeader::getExpiryTimestamp()
{
...
boost::shared_ptr<ImplementationSpecification> NotificationMessageHeader::getImplementationSpecification()
{
...
std::vector<boost::shared_ptr<PartyMessageInformation>> NotificationMessageHeader::getPartyMessageInformation()
{
...
boost::shared_ptr<SignatureType> NotificationMessageHeader::getSignature()
{
