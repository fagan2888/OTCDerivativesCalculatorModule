...
NotificationMessage::NotificationMessage(TiXmlNode* xmlNode)
: Message(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(headerNode)
   {
      if(headerNode->Attribute("href") || headerNode->Attribute("id"))
      {
          if(headerNode->Attribute("id"))
          {
             ...
             header_ = boost::shared_ptr<NotificationMessageHeader>(new NotificationMessageHeader(headerNode));
      ...
      else { header_ = boost::shared_ptr<NotificationMessageHeader>(new NotificationMessageHeader(headerNode));}
   }
...
boost::shared_ptr<NotificationMessageHeader> NotificationMessage::getHeader()
{
   if(!this->headerIsNull_){
        if(headerIDRef_ != ""){
             return boost::shared_static_cast<NotificationMessageHeader>(IDManager::instance().getID(headerIDRef_));
        }else{
             return this->header_;
        }
   }else
   {
      QL_FAIL("null Ptr");
      return boost::shared_ptr<NotificationMessageHeader>();
   }
}
std::vector<boost::shared_ptr<Validation>> NotificationMessage::getValidation()
{
...
boost::shared_ptr<CorrelationId> NotificationMessage::getParentCorrelationId()
{
...
boost::shared_ptr<CorrelationId> NotificationMessage::getCorrelationId()
{
...
boost::shared_ptr<XsdTypePositiveInteger> NotificationMessage::getSequenceNumber()
{
...
boost::shared_ptr<OnBehalfOf> NotificationMessage::getOnBehalfOf()
{
