// CoalAttributePercentage.cpp 
#include "CoalAttributePercentage.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
CoalAttributePercentage::CoalAttributePercentage(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(standardContentNode)
   {
      if(standardContentNode->Attribute("href") || standardContentNode->Attribute("id"))
      {
          if(standardContentNode->Attribute("id"))
          {
             ...
             standardContent_ = boost::shared_ptr<RestrictedPercentage>(new RestrictedPercentage(standardContentNode));
      ...
      else { standardContent_ = boost::shared_ptr<RestrictedPercentage>(new RestrictedPercentage(standardContentNode));}
   }
   ...
   if(rejectionLimitNode)
   {
      if(rejectionLimitNode->Attribute("href") || rejectionLimitNode->Attribute("id"))
      {
          if(rejectionLimitNode->Attribute("id"))
          {
             ...
             rejectionLimit_ = boost::shared_ptr<RestrictedPercentage>(new RestrictedPercentage(rejectionLimitNode));
      ...
      else { rejectionLimit_ = boost::shared_ptr<RestrictedPercentage>(new RestrictedPercentage(rejectionLimitNode));}
   }
...
boost::shared_ptr<RestrictedPercentage> CoalAttributePercentage::getStandardContent()
{
   if(!this->standardContentIsNull_){
        if(standardContentIDRef_ != ""){
             return boost::shared_static_cast<RestrictedPercentage>(IDManager::instance().getID(standardContentIDRef_));
        }else{
...
boost::shared_ptr<RestrictedPercentage> CoalAttributePercentage::getRejectionLimit()
{
   if(!this->rejectionLimitIsNull_){
        if(rejectionLimitIDRef_ != ""){
             return boost::shared_static_cast<RestrictedPercentage>(IDManager::instance().getID(rejectionLimitIDRef_));
        }else{
