// CommodityInformationSource.cpp 
#include "CommodityInformationSource.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
CommodityInformationSource::CommodityInformationSource(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(rateSourceNode)
   {
      if(rateSourceNode->Attribute("href") || rateSourceNode->Attribute("id"))
      {
          if(rateSourceNode->Attribute("id"))
          {
             ...
             rateSource_ = boost::shared_ptr<CommodityInformationProvider>(new CommodityInformationProvider(rateSourceNode));
      ...
      else { rateSource_ = boost::shared_ptr<CommodityInformationProvider>(new CommodityInformationProvider(rateSourceNode));}
   }
...
boost::shared_ptr<CommodityInformationProvider> CommodityInformationSource::getRateSource()
{
   if(!this->rateSourceIsNull_){
        if(rateSourceIDRef_ != ""){
             return boost::shared_static_cast<CommodityInformationProvider>(IDManager::instance().getID(rateSourceIDRef_));
        }else{
   ...
   {
      QL_FAIL("null Ptr");
      return boost::shared_ptr<CommodityInformationProvider>();
   }
}
boost::shared_ptr<RateSourcePage> CommodityInformationSource::getRateSourcePage()
{
...
boost::shared_ptr<XsdTypeString> CommodityInformationSource::getRateSourcePageHeading()
{
