// EventProposedMatch.cpp 
#include "EventProposedMatch.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
EventProposedMatch::EventProposedMatch(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   //matchIdNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* matchIdNode = xmlNode->FirstChildElement("matchId");

   if(matchIdNode){matchIdIsNull_ = false;}
   else{matchIdIsNull_ = true;}

   #ifdef ConsolePrint
      FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- matchIdNode , address : " << matchIdNode << std::endl;
   #endif
   if(matchIdNode)
   {
      if(matchIdNode->Attribute("href") || matchIdNode->Attribute("id"))
      {
          if(matchIdNode->Attribute("id"))
          {
             matchIdIDRef_ = matchIdNode->Attribute("id");
             matchId_ = boost::shared_ptr<MatchId>(new MatchId(matchIdNode));
             matchId_->setID(matchIdIDRef_);
             IDManager::instance().setID(matchIdIDRef_,matchId_);
          }
          else if(matchIdNode->Attribute("href")) { matchIdIDRef_ = matchIdNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { matchId_ = boost::shared_ptr<MatchId>(new MatchId(matchIdNode));}
   }

   //differenceNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* differenceNode = xmlNode->FirstChildElement("difference");

   if(differenceNode){differenceIsNull_ = false;}
   else{differenceIsNull_ = true;}

   if(differenceNode)
   {
      for(differenceNode; differenceNode; differenceNode = differenceNode->NextSiblingElement("difference")){
          #ifdef ConsolePrint
              FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- differenceNode , address : " << differenceNode << std::endl;
          #endif
          if(differenceNode->Attribute("href") || differenceNode->Attribute("id"))
          {
              if(differenceNode->Attribute("id"))
              {
                  differenceIDRef_ = differenceNode->Attribute("id");
                  difference_.push_back(boost::shared_ptr<TradeDifference>(new TradeDifference(differenceNode)));
                  difference_.back()->setID(differenceIDRef_);
                  IDManager::instance().setID(differenceIDRef_, difference_.back());
              }
              else if(differenceNode->Attribute("href")) { differenceIDRef_ = differenceNode->Attribute("href");}
              else { QL_FAIL("id or href error"); }
          }
          else { difference_.push_back(boost::shared_ptr<TradeDifference>(new TradeDifference(differenceNode)));}
      }
   }
   else {
       #ifdef ConsolePrint
   ...
   //matchScoreNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* matchScoreNode = xmlNode->FirstChildElement("matchScore");

   if(matchScoreNode){matchScoreIsNull_ = false;}
   else{matchScoreIsNull_ = true;}

   #ifdef ConsolePrint
      FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- matchScoreNode , address : " << matchScoreNode << std::endl;
   #endif
   if(matchScoreNode)
   {
      if(matchScoreNode->Attribute("href") || matchScoreNode->Attribute("id"))
      {
          if(matchScoreNode->Attribute("id"))
          {
             matchScoreIDRef_ = matchScoreNode->Attribute("id");
             matchScore_ = boost::shared_ptr<XsdTypeDecimal>(new XsdTypeDecimal(matchScoreNode));
             matchScore_->setID(matchScoreIDRef_);
             IDManager::instance().setID(matchScoreIDRef_,matchScore_);
          }
          else if(matchScoreNode->Attribute("href")) { matchScoreIDRef_ = matchScoreNode->Attribute("href");}
          else { QL_FAIL("id or href error"); }
      }
      else { matchScore_ = boost::shared_ptr<XsdTypeDecimal>(new XsdTypeDecimal(matchScoreNode));}
   }
...
boost::shared_ptr<OriginatingEvent> EventProposedMatch::getOriginatingEvent()
{
...
boost::shared_ptr<Trade> EventProposedMatch::getTrade()
{
...
boost::shared_ptr<TradeAmendmentContent> EventProposedMatch::getAmendment()
{
...
boost::shared_ptr<TradeNotionalChange> EventProposedMatch::getIncrease()
{
...
boost::shared_ptr<TerminatingEvent> EventProposedMatch::getTerminatingEvent()
{
...
boost::shared_ptr<TradeNotionalChange> EventProposedMatch::getTermination()
{
...
boost::shared_ptr<TradeNovationContent> EventProposedMatch::getNovation()
{
...
boost::shared_ptr<OptionExercise> EventProposedMatch::getOptionExercise()
{
...
std::vector<boost::shared_ptr<OptionExpiry>> EventProposedMatch::getOptionExpiry()
{
...
boost::shared_ptr<DeClear> EventProposedMatch::getDeClear()
{
...
boost::shared_ptr<Withdrawal> EventProposedMatch::getWithdrawal()
{
...
boost::shared_ptr<AdditionalEvent> EventProposedMatch::getAdditionalEvent()
{
...
boost::shared_ptr<MatchId> EventProposedMatch::getMatchId()
{
   if(!this->matchIdIsNull_){
        if(matchIdIDRef_ != ""){
             return boost::shared_static_cast<MatchId>(IDManager::instance().getID(matchIdIDRef_));
        }else{
   ...
   {
      QL_FAIL("null Ptr");
      return boost::shared_ptr<MatchId>();
   }
}
std::vector<boost::shared_ptr<TradeDifference>> EventProposedMatch::getDifference()
{
   if(!this->differenceIsNull_){
        if(differenceIDRef_ != ""){
...
boost::shared_ptr<XsdTypeDecimal> EventProposedMatch::getMatchScore()
{
   if(!this->matchScoreIsNull_){
        if(matchScoreIDRef_ != ""){
             return boost::shared_static_cast<XsdTypeDecimal>(IDManager::instance().getID(matchScoreIDRef_));
        }else{
