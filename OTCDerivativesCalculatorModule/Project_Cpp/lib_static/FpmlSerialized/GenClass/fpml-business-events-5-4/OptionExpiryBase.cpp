// OptionExpiryBase.cpp 
#include "OptionExpiryBase.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
OptionExpiryBase::OptionExpiryBase(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
...
std::vector<boost::shared_ptr<PartyTradeIdentifier>> OptionExpiryBase::getTradeIdentifier()
{
...
boost::shared_ptr<XsdTypeDate> OptionExpiryBase::getDate()
{
   if(!this->dateIsNull_){
        if(dateIDRef_ != ""){
...
boost::shared_ptr<XsdTypeTime> OptionExpiryBase::getTime()
{
   if(!this->timeIsNull_){
        if(timeIDRef_ != ""){
