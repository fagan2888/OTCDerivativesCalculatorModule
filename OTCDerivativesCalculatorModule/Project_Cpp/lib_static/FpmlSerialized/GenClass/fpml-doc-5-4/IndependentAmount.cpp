// IndependentAmount.cpp 
#include "IndependentAmount.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
IndependentAmount::IndependentAmount(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   //paymentDetailNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* paymentDetailNode = xmlNode->FirstChildElement("paymentDetail");

   if(paymentDetailNode){paymentDetailIsNull_ = false;}
   else{paymentDetailIsNull_ = true;}

   if(paymentDetailNode)
   {
      for(paymentDetailNode; paymentDetailNode; paymentDetailNode = paymentDetailNode->NextSiblingElement("paymentDetail")){
          #ifdef ConsolePrint
              FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- paymentDetailNode , address : " << paymentDetailNode << std::endl;
          #endif
          if(paymentDetailNode->Attribute("href") || paymentDetailNode->Attribute("id"))
          {
              if(paymentDetailNode->Attribute("id"))
              {
                  paymentDetailIDRef_ = paymentDetailNode->Attribute("id");
                  paymentDetail_.push_back(boost::shared_ptr<PaymentDetail>(new PaymentDetail(paymentDetailNode)));
                  paymentDetail_.back()->setID(paymentDetailIDRef_);
                  IDManager::instance().setID(paymentDetailIDRef_, paymentDetail_.back());
              }
              else if(paymentDetailNode->Attribute("href")) { paymentDetailIDRef_ = paymentDetailNode->Attribute("href");}
              else { QL_FAIL("id or href error"); }
          }
          else { paymentDetail_.push_back(boost::shared_ptr<PaymentDetail>(new PaymentDetail(paymentDetailNode)));}
      }
   }
   else {
       #ifdef ConsolePrint
...
boost::shared_ptr<PartyReference> IndependentAmount::getPayerPartyReference()
{
...
boost::shared_ptr<AccountReference> IndependentAmount::getPayerAccountReference()
{
...
boost::shared_ptr<PartyReference> IndependentAmount::getReceiverPartyReference()
{
...
boost::shared_ptr<AccountReference> IndependentAmount::getReceiverAccountReference()
{
...
std::vector<boost::shared_ptr<PaymentDetail>> IndependentAmount::getPaymentDetail()
{
   if(!this->paymentDetailIsNull_){
        if(paymentDetailIDRef_ != ""){
   ...
   {
      QL_FAIL("null Ptr");
      return std::vector<boost::shared_ptr<PaymentDetail>>();
   }
}
}
