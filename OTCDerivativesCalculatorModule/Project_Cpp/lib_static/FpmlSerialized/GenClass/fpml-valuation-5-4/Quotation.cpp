...
Quotation::Quotation(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   //sensitivitySetNode ----------------------------------------------------------------------------------------------------------------------
   TiXmlElement* sensitivitySetNode = xmlNode->FirstChildElement("sensitivitySet");

   if(sensitivitySetNode){sensitivitySetIsNull_ = false;}
   else{sensitivitySetIsNull_ = true;}

   if(sensitivitySetNode)
   {
      for(sensitivitySetNode; sensitivitySetNode; sensitivitySetNode = sensitivitySetNode->NextSiblingElement("sensitivitySet")){
          #ifdef ConsolePrint
              FileManager::instance().outFile_ << FileManager::instance().tap_.c_str() << "- sensitivitySetNode , address : " << sensitivitySetNode << std::endl;
          #endif
          if(sensitivitySetNode->Attribute("href") || sensitivitySetNode->Attribute("id"))
          {
              if(sensitivitySetNode->Attribute("id"))
              {
                  sensitivitySetIDRef_ = sensitivitySetNode->Attribute("id");
                  sensitivitySet_.push_back(boost::shared_ptr<SensitivitySet>(new SensitivitySet(sensitivitySetNode)));
                  sensitivitySet_.back()->setID(sensitivitySetIDRef_);
                  IDManager::instance().setID(sensitivitySetIDRef_, sensitivitySet_.back());
              }
              else if(sensitivitySetNode->Attribute("href")) { sensitivitySetIDRef_ = sensitivitySetNode->Attribute("href");}
              else { QL_FAIL("id or href error"); }
          }
          else { sensitivitySet_.push_back(boost::shared_ptr<SensitivitySet>(new SensitivitySet(sensitivitySetNode)));}
      }
   }
   else {
       #ifdef ConsolePrint
...
boost::shared_ptr<XsdTypeDecimal> Quotation::getValue()
{
...
boost::shared_ptr<AssetMeasureType> Quotation::getMeasureType()
{
...
boost::shared_ptr<PriceQuoteUnits> Quotation::getQuoteUnits()
{
...
boost::shared_ptr<QuotationSideEnum> Quotation::getSide()
{
   if(!this->sideIsNull_){
        if(sideIDRef_ != ""){
...
boost::shared_ptr<Currency> Quotation::getCurrency()
{
...
boost::shared_ptr<ReportingCurrencyType> Quotation::getCurrencyType()
{
...
boost::shared_ptr<QuoteTiming> Quotation::getTiming()
{
...
boost::shared_ptr<BusinessCenter> Quotation::getBusinessCenter()
{
...
boost::shared_ptr<ExchangeId> Quotation::getExchangeId()
{
...
std::vector<boost::shared_ptr<InformationSource>> Quotation::getInformationSource()
{
...
boost::shared_ptr<PricingModel> Quotation::getPricingModel()
{
...
boost::shared_ptr<XsdTypeDateTime> Quotation::getTime()
{
   if(!this->timeIsNull_){
        if(timeIDRef_ != ""){
...
boost::shared_ptr<XsdTypeDate> Quotation::getValuationDate()
{
...
boost::shared_ptr<XsdTypeDateTime> Quotation::getExpiryTime()
{
...
boost::shared_ptr<CashflowType> Quotation::getCashflowType()
{
...
std::vector<boost::shared_ptr<SensitivitySet>> Quotation::getSensitivitySet()
{
   if(!this->sensitivitySetIsNull_){
        if(sensitivitySetIDRef_ != ""){
             return this->sensitivitySet_;
        }else{
             return this->sensitivitySet_;
        }
   }else
   {
      QL_FAIL("null Ptr");
      return std::vector<boost::shared_ptr<SensitivitySet>>();
   }
}
}
