...
FxStrikePrice::FxStrikePrice(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(strikeQuoteBasisNode)
   {
      if(strikeQuoteBasisNode->Attribute("href") || strikeQuoteBasisNode->Attribute("id"))
      {
          if(strikeQuoteBasisNode->Attribute("id"))
          {
             ...
             strikeQuoteBasis_ = boost::shared_ptr<StrikeQuoteBasisEnum>(new StrikeQuoteBasisEnum(strikeQuoteBasisNode));
      ...
      else { strikeQuoteBasis_ = boost::shared_ptr<StrikeQuoteBasisEnum>(new StrikeQuoteBasisEnum(strikeQuoteBasisNode));}
   }
...
boost::shared_ptr<PositiveDecimal> FxStrikePrice::getRate()
{
   if(!this->rateIsNull_){
        if(rateIDRef_ != ""){
...
boost::shared_ptr<StrikeQuoteBasisEnum> FxStrikePrice::getStrikeQuoteBasis()
{
   if(!this->strikeQuoteBasisIsNull_){
        if(strikeQuoteBasisIDRef_ != ""){
             return boost::shared_static_cast<StrikeQuoteBasisEnum>(IDManager::instance().getID(strikeQuoteBasisIDRef_));
        }else{
