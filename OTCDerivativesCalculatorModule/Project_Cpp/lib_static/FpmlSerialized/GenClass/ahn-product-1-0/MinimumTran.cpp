...
MinimumTran::MinimumTran(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
      ...
      for(variableNode; variableNode; variableNode = variableNode->NextSiblingElement("variable")){
          ...
          if(variableNode->Attribute("href") || variableNode->Attribute("id"))
          {
              if(variableNode->Attribute("id"))
              {
                  ...
                  variable_.push_back(boost::shared_ptr<VariableValue>(new VariableValue(variableNode)));
          ...
          else { variable_.push_back(boost::shared_ptr<VariableValue>(new VariableValue(variableNode)));}
      }
   }
   else {
       #ifdef ConsolePrint
...
boost::shared_ptr<XsdTypeToken> MinimumTran::getSymbolName()
{
...
std::vector<boost::shared_ptr<VariableValue>> MinimumTran::getVariable()
{
   ...
   {
      QL_FAIL("null Ptr");
      return std::vector<boost::shared_ptr<VariableValue>>();
   }
}
}
