// NonNegativeSchedule.cpp 
#include "NonNegativeSchedule.hpp"
#ifdef ConsolePrint
    #include <iostream>
#endif 
...
NonNegativeSchedule::NonNegativeSchedule(TiXmlNode* xmlNode)
: ISerialized(xmlNode)
{
    #ifdef ConsolePrint
   ...
   if(initialValueNode)
   {
      if(initialValueNode->Attribute("href") || initialValueNode->Attribute("id"))
      {
          if(initialValueNode->Attribute("id"))
          {
             ...
             initialValue_ = boost::shared_ptr<NonNegativeDecimal>(new NonNegativeDecimal(initialValueNode));
      ...
      else { initialValue_ = boost::shared_ptr<NonNegativeDecimal>(new NonNegativeDecimal(initialValueNode));}
   }
      ...
      for(stepNode; stepNode; stepNode = stepNode->NextSiblingElement("step")){
          ...
          if(stepNode->Attribute("href") || stepNode->Attribute("id"))
          {
              if(stepNode->Attribute("id"))
              {
                  ...
                  step_.push_back(boost::shared_ptr<NonNegativeStep>(new NonNegativeStep(stepNode)));
          ...
          else { step_.push_back(boost::shared_ptr<NonNegativeStep>(new NonNegativeStep(stepNode)));}
      }
   }
   else {
       #ifdef ConsolePrint
...
boost::shared_ptr<NonNegativeDecimal> NonNegativeSchedule::getInitialValue()
{
   if(!this->initialValueIsNull_){
        if(initialValueIDRef_ != ""){
             return boost::shared_static_cast<NonNegativeDecimal>(IDManager::instance().getID(initialValueIDRef_));
        }else{
...
std::vector<boost::shared_ptr<NonNegativeStep>> NonNegativeSchedule::getStep()
{
   if(!this->stepIsNull_){
        if(stepIDRef_ != ""){
             return this->step_;
   ...
   {
      QL_FAIL("null Ptr");
      return std::vector<boost::shared_ptr<NonNegativeStep>>();
   }
}
}
