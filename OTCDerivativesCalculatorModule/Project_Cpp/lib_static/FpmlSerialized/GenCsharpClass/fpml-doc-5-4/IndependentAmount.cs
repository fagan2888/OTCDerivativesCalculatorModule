    ...
    public class IndependentAmount
    {
        public IndependentAmount(XmlNode xmlNode)
        {
            ...
            foreach (XmlNode item in paymentDetailNodeList)
            {
                if (item.Attributes["href"] != null || item.Attributes["id"] == null) 
                {
                    if (item.Attributes["id"] != null) 
                    {
                        paymentDetailIDRef = item.Attributes["id"].Name;
                        List<PaymentDetail> ob = new List<PaymentDetail>();
                        ob.Add(new PaymentDetail(item));
                        IDManager.SetID(paymentDetailIDRef, ob);
                    }
                    else if (item.Attributes.ToString() == "href")
                    {
                        paymentDetailIDRef = item.Attributes["href"].Name;
                    }
                    else
                    {
                    paymentDetail.Add(new PaymentDetail(item));
                    }
                }
            }
        ...
        public List<PaymentDetail> PaymentDetail
        {
            get
            {
                ...
                else if (this.paymentDetailIDRef != null)
                {
                    paymentDetail = IDManager.getID(paymentDetailIDRef) as List<PaymentDetail>;
                    return this.paymentDetail; 
                }
                else
                {
        ...
        public string PaymentDetailIDRef { get; set; }
