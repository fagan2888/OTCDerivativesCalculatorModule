...
{
    public class IndependentAmount : ISerialized
    {
        public IndependentAmount(XmlNode xmlNode)
        {
            ...
            XmlNodeList paymentDetailNodeList = xmlNode.SelectNodes("paymentDetail");
            
            if (paymentDetailNodeList != null)
            {
                this.paymentDetail_ = new List<PaymentDetail>();
                foreach (XmlNode item in paymentDetailNodeList)
                {
                    if (item.Attributes["href"] != null || item.Attributes["id"] != null) 
                    {
                        if (item.Attributes["id"] != null) 
                        {
                            paymentDetailIDRef_ = item.Attributes["id"].Value;
                            paymentDetail_.Add(new PaymentDetail(item));
                            IDManager.SetID(paymentDetailIDRef_, paymentDetail_[paymentDetail_.Count - 1 ]);
                        }
                        else if (item.Attributes["href"] != null)
                        {
                            paymentDetailIDRef_ = item.Attributes["href"].Value;
                        }
                        else
                        {
                        paymentDetail_.Add(new PaymentDetail(item));
        ...
        #region PaymentDetail_
        private List<PaymentDetail> paymentDetail_;
        public List<PaymentDetail> PaymentDetail_
        {
            get
            {
                if (this.paymentDetail_ != null)
                {
                    return this.paymentDetail_; 
                }
                else if (this.paymentDetailIDRef_ != null)
                {
                    return this.paymentDetail_; 
                }
                else
            ...
            set
            {
                if (this.paymentDetail_ != value)
                {
                    this.paymentDetail_ = value;
                }
            }
        }
        #endregion
        
        public string paymentDetailIDRef_ { get; set; }
