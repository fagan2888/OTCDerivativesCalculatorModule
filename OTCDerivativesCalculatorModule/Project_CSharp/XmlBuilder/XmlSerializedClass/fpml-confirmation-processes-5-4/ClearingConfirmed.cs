...
{
    public class ClearingConfirmed : NotificationMessage
    {
        public ClearingConfirmed(XmlNode xmlNode)
        : base(xmlNode)
        {
            XmlNode compressionActivityNode = xmlNode.SelectSingleNode("compressionActivity");
            
            if (compressionActivityNode != null)
            {
                if (compressionActivityNode.Attributes["href"] != null || compressionActivityNode.Attributes["id"] != null) 
                {
                    if (compressionActivityNode.Attributes["id"] != null) 
                    {
                        compressionActivityIDRef_ = compressionActivityNode.Attributes["id"].Value;
                        CompressionActivity ob = new CompressionActivity(compressionActivityNode);
                        IDManager.SetID(compressionActivityIDRef_, ob);
                    }
                    else if (compressionActivityNode.Attributes["href"] != null)
                    {
                        compressionActivityIDRef_ = compressionActivityNode.Attributes["href"].Value;
                    }
                    else
                    {
                        compressionActivity_ = new CompressionActivity(compressionActivityNode);
                    }
                }
                else
                {
            ...
            if (portfolioReferenceNode != null)
            {
                if (portfolioReferenceNode.Attributes["href"] != null || portfolioReferenceNode.Attributes["id"] != null) 
                {
                    if (portfolioReferenceNode.Attributes["id"] != null) 
                    {
                        ...
                        PortfolioReference ob = new PortfolioReference(portfolioReferenceNode);
                    ...
                    {
                        portfolioReference_ = new PortfolioReference(portfolioReferenceNode);
                    }
                }
                else
                {
            ...
            XmlNode clearingNode = xmlNode.SelectSingleNode("clearing");
            
            if (clearingNode != null)
            {
                if (clearingNode.Attributes["href"] != null || clearingNode.Attributes["id"] != null) 
                {
                    if (clearingNode.Attributes["id"] != null) 
                    {
                        clearingIDRef_ = clearingNode.Attributes["id"].Value;
                        Clearing ob = new Clearing(clearingNode);
                        IDManager.SetID(clearingIDRef_, ob);
                    }
                    else if (clearingNode.Attributes["href"] != null)
                    {
                        clearingIDRef_ = clearingNode.Attributes["href"].Value;
                    }
                    else
                    {
                        clearing_ = new Clearing(clearingNode);
                    }
                }
                else
                {
        ...
        #region CompressionActivity_
        private CompressionActivity compressionActivity_;
        public CompressionActivity CompressionActivity_
        {
            get
            {
                if (this.compressionActivity_ != null)
                {
                    return this.compressionActivity_; 
                }
                else if (this.compressionActivityIDRef_ != null)
                {
                    compressionActivity_ = IDManager.getID(compressionActivityIDRef_) as CompressionActivity;
                    return this.compressionActivity_; 
                }
                else
                {
            ...
            set
            {
                if (this.compressionActivity_ != value)
                {
                    this.compressionActivity_ = value;
                }
            }
        }
        #endregion
        
        public string compressionActivityIDRef_ { get; set; }
        #region PortfolioReference_
        private PortfolioReference portfolioReference_;
        public PortfolioReference PortfolioReference_
        {
            get
            {
                ...
                {
                    portfolioReference_ = IDManager.getID(portfolioReferenceIDRef_) as PortfolioReference;
                    return this.portfolioReference_; 
                }
                else
                {
        ...
        #region Clearing_
        private Clearing clearing_;
        public Clearing Clearing_
        {
            get
            {
                if (this.clearing_ != null)
                {
                    return this.clearing_; 
                }
                else if (this.clearingIDRef_ != null)
                {
                    clearing_ = IDManager.getID(clearingIDRef_) as Clearing;
                    return this.clearing_; 
                }
                else
                {
            ...
            set
            {
                if (this.clearing_ != value)
                {
                    this.clearing_ = value;
                }
            }
        }
        #endregion
        
        public string clearingIDRef_ { get; set; }
        #region Quote_
