...
{
    public class Quotation : ISerialized
    {
        public Quotation(XmlNode xmlNode)
        {
            ...
            if (sensitivitySetNodeList != null)
            {
                this.sensitivitySet_ = new List<SensitivitySet>();
                foreach (XmlNode item in sensitivitySetNodeList)
                {
                    if (item.Attributes["href"] != null || item.Attributes["id"] != null) 
                    {
                        if (item.Attributes["id"] != null) 
                        {
                            sensitivitySetIDRef_ = item.Attributes["id"].Value;
                            sensitivitySet_.Add(new SensitivitySet(item));
                            IDManager.SetID(sensitivitySetIDRef_, sensitivitySet_[sensitivitySet_.Count - 1 ]);
                        }
                        else if (item.Attributes["href"] != null)
                        {
                            sensitivitySetIDRef_ = item.Attributes["href"].Value;
                        }
                        else
                        {
                        sensitivitySet_.Add(new SensitivitySet(item));
        ...
        #region SensitivitySet_
        private List<SensitivitySet> sensitivitySet_;
        public List<SensitivitySet> SensitivitySet_
        {
            get
            {
                if (this.sensitivitySet_ != null)
                {
                    return this.sensitivitySet_; 
                }
                else if (this.sensitivitySetIDRef_ != null)
                {
                    return this.sensitivitySet_; 
                }
                else
            ...
            set
            {
                if (this.sensitivitySet_ != value)
                {
                    this.sensitivitySet_ = value;
                }
            }
        }
        #endregion
        
        public string sensitivitySetIDRef_ { get; set; }
